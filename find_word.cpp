...
Find_Word::Find_Word()
{
    loadCoreset("coresetORDINATO.txt");
    prevViseme.power=0;
    prevViseme.value=-1;
}
...
// return: true=word complete, false=word uncomplete.
{
    ...
    {
        prevViseme.power++;
        if ( prevViseme.value==9 && prevViseme.power>=5 && visemeList.size()>1)
...
vector<string> visemeMap(int visemeCode)
{
    ...
    {                
        ...
        case(3):                // O_CO_GO
            ...
            vChar.push_back("co");
            vChar.push_back("go");
            //visemeChar.push_back("to");
            //visemeChar.push_back("so");
            //visemeChar.push_back("do");
        break;
